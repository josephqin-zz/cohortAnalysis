...
class Panel extends React.Component{
    constructor(props){
        super(props);
        this.state={showRows:props.data.filter((d,i)=>i<=1).map((d)=>d.name),elms:props.data[0].values.map((d,index)=>index)}
    }

    removeFn(entryName){
       this.setState((prestate)=>({showRows:prestate.showRows.filter((d)=>d!==entryName)}))
    }
    selectedRowFn(key,chooseValue){
       if(chooseValue.length>0)this.setState({showRows:chooseValue})
    }
    updateFn(key,chooseValue){
        this.setState({elms:this.props.data.filter((d)=>d.name===key)[0].values.map((d,index)=>chooseValue.includes(d)?index:-1).filter((d)=>d>-1)})
    ...
    render(){
        const colsOptions = {name:'rows',range:this.props.data.map(d=>d.name).map(d=>({value:d,label:d}))}
        ...
        <div>
            ...
            <Selection data={colsOptions} updateFn={this.selectedRowFn.bind(this)} />
            <Menu updateFn={this.updateFn.bind(this)} data={dataset.filter((d)=>d.type!=='number').map((d)=>{ let {name,range}=d; range=range.map(d=>({value:d,label:d})); return{name,range} })}/>
